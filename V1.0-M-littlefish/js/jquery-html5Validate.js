...
(function($, undefined) {
  ...
  OBJREG = {
    ...
    "prompt": {
      ...
      checkbox: "If must continue, please check this box",
      "select": "Please select an item in the list",
      email: "Please enter an email address",
      url: "Please enter the website address",
      tel: "Please enter the mobile phone number",
      number: "Please enter a value number",
      date: "Please enter a date",
      pattern: "The content meets the formatting requirements",
      empty: "Please fill in this field",
      multiple: "Separate the event names with commas"
    }
  };
      ...
      isOverflow: function(ele) {
        ...
        if (!attrMin && !attrMax) {
          ...
          if (attrDataMin && value.length < attrDataMin) {
            $(ele).testRemind("You must enter at least " + attrDataMin + "character");
            ele.focus();
          } else if (attrDataMax && value.length > attrDataMax) {
            $(ele).testRemind("You may enter up to " + attrDataMax + "character");
          ...
          if (attrMin && value < attrMin) {
            $(ele).testRemind("The value must be greater than or equal to" + attrMin);
          } else if (attrMax && value > attrMax) {
            $(ele).testRemind("The value must be less than or equal to" + attrMax);
          } else if (attrStep && !/^\d+(\.0+)?$/.test((Math.abs((value - attrMin || 0)) / attrStep).toFixed(10))) {
            $(ele).testRemind("Invalid value");
          } else {
            return false;
          }
          ele.focus();
            ...
            , remind = function(control, type, tag) {
              ...
              if ($(control).isVisible()) {
                if (type == "radio" || type == "checkbox") {
                  ...
                  // 下拉值为空或文本框文本域等为空
                  $(control).testRemind((tag == "empty" && text)? "Please enter your "+ text : OBJREG.prompt[tag]);
                  control.focus();
                } else if (/^range|number$/i.test(type) && Number(control.value)) {
                  // 整数值与数值的特殊提示
                  $(control).testRemind("Invalid value");
                  control.focus();
                  control.select();
                } else {
                  // 文本框文本域格式不准确
                  ...
                  if (text) {
                    finalText = "Please enter a valid "+ text +" address";
                  }
                ...
                var customTxt = "You have not " + (key || (tag == "empty"? "entered": "selected")) + ((!/^radio|checkbox$/i.test(type) && text) || "this item");
                if (target.size()) {
